...
export const KICKOFF_BEAT_1_PRIMARY = [
  "Before we map out this track, I want to know where you're coming from. What happened last track that leads us here?",
...
export const KICKOFF_BEAT_2_PRIMARY = [
  "What's the goal for this track?",
  "Why does that matter right now?",
  "What does success look like when this track is done?",
...
export const KICKOFF_BEAT_2_PROBES = {
  ...
  success: "If I checked in on the last day of this track, what would you show me?",
} as const;

// ── Beat 3 — The Stakes ───────────────────────────────────────────────────────

export const KICKOFF_BEAT_3_PRIMARY = [
  "What's the biggest risk or unknown heading into this track?",
...
Based on the kickoff conversation above, write a single declarative sentence that captures what this track is really about. The sentence should go beyond the task and capture the underlying tension or opportunity. It should be honest, specific, and human. It is not a goal statement — it is a narrative frame.

Examples of good thesis sentences:
- "This track is about finding out if strangers will pay for what friends have praised."
- "This track is really about whether the product can survive without the founder doing everything."
- "This is the track where the gap between the plan and reality either closes or becomes permanent."

Output only the thesis sentence. No explanation.
`.trim();

export const kickoffThesisConfirmation = (thesis: string): string =>
  `So if I had to capture what this track is really about in one sentence, I'd say: "${thesis}". Does that feel right, or would you change it?`;

// ── Re-centering injections (added after standard beats when flag is set) ──────

export const RECENTERING_INJECTIONS = {
  flatline: `After you ask the goal question, add:\n\n"Before we go any further — I want to zoom out for a second. You've been heads down executing consistently, and that's real. But I'm curious: has anything shifted in how you think about what you're building? Sometimes the most important track isn't about a new task. It's about a new perspective."`,

  missing_antagonist: `After the stakes beat, add:\n\n"One thing I want to make sure we capture — what's the thing that's most in your way right now? Not a task. The actual obstacle. The thing you're really fighting against this track."`,

  shrinking_stakes: `After the stakes beat, add:\n\n"We haven't talked about this in a few tracks, so I want to ask directly: why does this still matter to you personally? Not the business case — the personal one. What would it mean to you if this works?"`,
