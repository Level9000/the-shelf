...
You are Cass — the story guide inside Authored By. You are running a track retrospective. Your job is to help founders reflect honestly on a track that just happened. You are warm, direct, and genuinely curious. You ask one question at a time. You are not looking for a polished summary — you are looking for the truth. The gap between what was planned and what actually happened is the most interesting thing in any retro. So are surprises, frustrations, unexpected wins, and shifts in thinking. Ask follow-up questions when answers are thin. Never accept "it went fine" as a complete answer. The raw material you collect here is what the storytelling system uses to write a track worth reading.
...
export const retroAccountingOpener = (
  ...
  const droppedLine =
    droppedTitles.length > 0
      ...
      : "";
  return `Okay, let's look at how this track actually went. You completed ${completedCount} of ${totalCount} cards.${completedLine}${droppedLine}\n\nOn a scale of 1–5, how would you rate this track overall?`;
};
...
export const RETRO_BEAT_2_QUESTIONS = {
  biggest_surprise:     "What surprised you most this track — good or bad?",
...
export const RETRO_BEAT_3_QUESTIONS = {
  new_knowledge:        "What do you know now that you didn't know when you started this track?",
  thinking_shift:       "Did your thinking change at all — about the product, the market, or yourself?",
  would_do_differently: "If you ran this track again with what you know now, what would you do differently?",
} as const;
...
export const retroGutFeelDelta = (kickoffGutFeeling: string): string =>
  `At kickoff you said your gut feeling going in was: "${kickoffGutFeeling}". How does that compare to how you feel now that the track is done?`;

export const RETRO_BEAT_4_QUESTIONS = {
  road_ahead:           "How are you feeling about the road ahead coming out of this track?",
...
Based on the retro conversation above, write a single sentence that captures what the next track is really about. It should follow naturally from what this track revealed. It should create forward momentum without being falsely optimistic. It should feel like the beginning of the next story beat, not a summary of this one.

Output only the bridge sentence. No explanation.
`.trim();

export const retroBridgeConfirmation = (bridge: string): string =>
  `Based on everything you just told me, it sounds like the next track is really about: "${bridge}". Does that feel right as a starting point for next time?`;
...
export const RECENTERING_RETRO_INJECTIONS = {
  ...
  fog_spiral_learning: `After the learning beat, add:\n\n"Let me ask you something directly. How are you actually feeling about where things are right now? Not the progress — you. Because sometimes the most important track is the one that captures what it costs to keep going."`,
} as const;
