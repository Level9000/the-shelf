...
import { runPressGapAnalysis } from "@/lib/ai/anthropic";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await runPressGapAnalysis({
      messages,
    ...
    console.error("Press gap analysis failed", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Press gap analysis failed." },
      { status: 500 },
    );
  }
}
