...
import { runShareEmailGeneration } from "@/lib/ai/anthropic";
...
export async function POST(request: Request) {
  ...
  const audienceType = String(payload.audienceType ?? "Team");
  ...
  const [{ data: project }, { data: board }] = await Promise.all([
    ...
    supabase
      .from("boards")
      .select("id,name,goal,why_it_matters,chapter_story,retro_completed_at")
      .eq("id", chapterId)
  ...
  try {
    const result = await runShareEmailGeneration({
      messages,
    ...
    console.error("Share email generation failed", error);
    return NextResponse.json(
