...
export function buildCassOnboardingPrompt(): string {
  return [
    ...
    "  \"Great, so tell me about your business journey so far.\"",
    ...
    "When the user answers this question, set done=true.",
    ...
    "- done: true only after the user has answered step 4",
    ...
    "- project_biggest_risk: the main unknown or risk, from step 3 (empty string while gathering)",
    "- proposed_chapters: [{chapter_number: 1, title, goal, prefill: {goal, value, measure, done}}]",
    "  (empty array while gathering; exactly 1 chapter when done=true, seeded from the step 4 answer)",
    "",
    "Return JSON only — no prose outside the JSON structure.",
  ].join("\n");
}
