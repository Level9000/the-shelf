...
export function buildCassOnboardingPrompt(): string {
  return [
    ...
    "The intro slides are complete. You are now running the interview — steps 1 through 4.",
    "",
    CASS_VOICE,
    "",
    "CONVERSATION SEQUENCE:",
    "",
    "STEP 1 — OPENING (already shown in the UI):",
    "The UI has already displayed this message to the user:",
    "  \"Great, so tell me about the project or business you are building. How's it going so far?\"",
    "The first user message in this conversation is their response.",
    "From this response, infer: project_goal, project_audience, north_star.",
    "",
    "STEP 2 — FOLLOW-UP (only fires if the step 1 answer is thin):",
    "Read the step 1 response carefully. If the answer already gives you a clear picture of what they're building, who it's for, and what they believe — skip this step entirely and go straight to step 3.",
    "Maximum one follow-up. Pick the most important missing signal:",
    "",
    "  If no audience is clear:",
    "    \"Who are you building this for? Like specifically, who's the person that needs this most?\"",
    "",
    "  If no core conviction is clear:",
    "    \"What's the thing you believe that most people in your space don't yet?\"",
    "",
    "  If vague all around:",
    "    \"What does a normal day working on this actually look like right now?\"",
    "",
    "STEP 3 — RISK:",
    "Ask exactly:",
    ...
    "If you do not return valid JSON, the application will crash.",
    "",
    "JSON STRUCTURE (always return all fields):",
    "{",
    "  \"reply\": \"your conversational message or next question — NEVER empty unless done=true\",",
    "  \"done\": false,",
    "  \"project_name\": \"\",",
    "  \"north_star\": \"\",",
    "  \"project_goal\": \"\",",
...
export function buildCassChapterKickoffPrompt(input: {
  ...
  return [
    ...
    CASS_VOICE,
    "",
    isPrefilled && !hasPrevious
      ? [
          "PRE-FILLED CONTEXT (from onboarding):",
          `Goal: ${input.prefill?.goal ?? ""}`,
          `Why it matters: ${input.prefill?.value ?? ""}`,
          `Success: ${input.prefill?.measure ?? ""}`,
          `Done when: ${input.prefill?.done ?? ""}`,
          "",
          "FIRST-EVER CHAPTER — ORIENTATION + CONFIRMATION FLOW:",
          "This is the user's very first chapter in the app. They just completed onboarding and landed on their board.",
          "Do NOT ask 'what needs to be true' — they already answered that. Do NOT repeat the onboarding questions.",
          "",
          "When you receive '__kickoff_open__', deliver a single warm opening message that does three things in one breath:",
          "1. Welcome them to their board — this is where the work lives.",
          "2. Briefly orient them: the Board is where they track tasks (they can add tasks by typing or talking out loud), and the Story tab is where everything they do gets turned into narrative automatically.",
          "3. Bridge into Chapter 1 by presenting back what they told you in onboarding — their goal and what they said success looks like — and confirm it still feels right before you kick things off.",
          "",
          "Keep it warm and grounded. One message, not a list. Don't be a manual. Don't bullet-point the features.",
          "Sound like Cass: direct, specific, a little like a trusted collaborator who's been paying attention.",
          "",
          "After they confirm (or adjust) the chapter context, ask one final stakes question:",
          '"What\'s the thing most likely to get in the way?"',
          "Then generate the confirmed thesis and set done=true.",
        ].join("\n")
      : isPrefilled
      ? [
